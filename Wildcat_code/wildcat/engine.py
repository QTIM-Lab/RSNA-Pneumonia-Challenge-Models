'''
Runs the actual training, handles validation and saving of models
'''
import os
import shutil
import time
...
class Engine(object):
    def __init__(self, state={}):
        ...
        # add accuracies and losses for iterations, rather than just epochs
        self.state['iteration_losses'] = []
        self.state['iteration_accuracies'] = []
    ...
    def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
        ...
        for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
            ...
            # is_best = True
            # is_best determined by the loss of the model on the validation set ('meter_loss')
            # is_best = True -> model is saved, else model is not saved
            if self.state['meter_loss'].value()[0] < performance_measure:
                print("updating model!")
                performance_measure = self.state['meter_loss'].value()[0]
                print(self.state['meter_loss'])
            ...
            if epoch - starting_epoch >= 5:
                s = "Stopped at " + str(epoch)
                with open("./early_saving.txt", 'w+') as f:
                    f.write(s)
                print("breaking")
                break
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch, val_loader):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            # this checks validation every 100 iterations. Not used for pneumonia model
            # if i % 100 == 0:
                # self.validate(val_loader, model, criterion, True)

        # print(self.state['iteration_losses'])
        # print(self.state['iteration_accuracies'])
    ...
    def validate(self, data_loader, model, criterion, is_training=False):
        ...
        # if is_training:
        #     print("adding values!")
        #     self.state['iteration_losses'].append(self.state['meter_loss'].value()[0])
        #     self.state['iteration_accuracies'].append(score)
        #     if self.state['meter_loss'].value()[0] < loss:
        #         self.save_checkpoint({
        #         'epoch': 0,
        #         'arch': self._state('arch'),
        #         'state_dict': model.module.state_dict() if self.state['use_gpu'] else model.state_dict(),
...
class MulticlassEngine(Engine):
    ...
    def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        ...
        # add measurement of iteration loss to new states
