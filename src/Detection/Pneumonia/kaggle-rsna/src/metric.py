...
def iou(box1, box2):
    ...
    area1, area2 = (w1+1) * (h1+1), (w2+1) * (h2+1)
    xi1, yi1, xi2, yi2 = max([x11, x21]), max([y11, y21]), min([x12, x22]), min([y12, y22])

    if xi2 <= xi1 or yi2 <= yi1:
        return 0
    else:
        intersect = (xi2-xi1+1) * (yi2-yi1+1)

        # uncomment this for localization
        # return intersect/area2

        # return for Kaggle RSNA Detection mAP score (actual iou)
...
def map_iou(boxes_true, boxes_pred, scores, thresholds=(0.4, 0.45, 0.5, 0.55, 0.6, 0.65, 0.7, 0.75)):
    ...
    return 0.0, map_total / len(thresholds)
