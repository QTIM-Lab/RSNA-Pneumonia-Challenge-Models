...
class DetectionDataset(Dataset):
    ...
    def __init__(self, fold: int, is_training: bool, debug: bool, img_size: int, csv: str, augmentation_level=10, crop_source=1024):
        """
        Args:
        ...
        self.categories = ["Normal", "Not Normal, Not Pneumonia", "Pneumonia"]
        self.samples = pd.read_csv(csv)

        # self.samples = samples[samples['Target'] == 1]
        print("Debug mode, samples: ", self.samples.head(12))
    ...
    def get_image(self, patient_id):
        ...
        Path has been changed for our dicom images"""
        dcm_data = pydicom.read_file(f"{TRAIN_DIR}/{patient_id}.dcm")
        img = dcm_data.pixel_array
        adjusted_image = cv2.equalizeHist(img)
...
def test_dataset_sample(sample_num=0):
    ...
    """
    dataset = DetectionDataset(fold, is_training=True, debug=False, img_size=224)
...
def test_augmentations(sample_num=12):
    ...
    Args:
        sample_num: sample number from the dataset
    """
    fold = "../../../Supervised_100_2detect2.csv"

    with timeit_context("load ds"):
        ds = DetectionDataset(fold, is_training=True, debug=True, img_size=224, augmentation_level=20)
        # print and plot sample
        print(ds[sample_num])
        plt.figure()
...
def plot_augmented_image(sample_num: int, aug_level=20, save=False):
    ...
    """
    fold = "../../../Supervised_100_2detect2.csv"
    ds = DetectionDataset(fold, is_training=True, debug=True, img_size=224, augmentation_level=aug_level)
    print(ds[sample_num])
...
def test_performance():
    ...
    fold = "../../../Supervised_100_2detect2.csv"
    with timeit_context("load ds"):
        ds = DetectionDataset(fold, is_training=True, debug=False, img_size=224)
